...
/**
 * Two decimals, then trailing zeros dropped.
 *
 * One decimal was wrong, not merely terse: a 0.75-credit sessional printed as
 * "0.8" and an 11.25-credit semester as "11.3". A credit figure that does not
 * match the department's own document is worse than an ugly one.
 */
...
function creditsOf(courses) {
  return courses.reduce((sum, c) => sum + (c.total ?? 0), 0);
}
...
function courseTable(doc, semester) {
  ...
  for (const course of semester.courses) {
    ...
    doc
      ...
      .fontSize(9)
      .fillColor(INK)
      .text(formatCredits(course.total), left + codeW + titleW, y + 5, { width: creditW - 6, align: 'right' });
...
function creditTable(doc, rows) {
  const left = PAGE_MARGIN;
  const width = doc.page.width - PAGE_MARGIN * 2;
  const columns = ['Semester', 'Theory', 'Sessional', 'Total', 'Cumulative'];
  const semesterW = width - 4 * 70;
  const widths = [semesterW, 70, 70, 70, 70];
  ...
  for (const row of rows) {
    ...
    const cells = [
      row.semester,
      formatCredits(row.theory),
      formatCredits(row.sessional),
