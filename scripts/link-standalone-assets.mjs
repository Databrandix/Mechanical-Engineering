 ...
 *
 * Uploads are not a concern here the way they are on the main su.edu.bd site:
 * this site stores CMS media on Cloudinary (CLOUDINARY_* env), so nothing is
 * written into public/ at runtime and there is no persistent upload directory
 * to keep outside `.next/`.
 *
 ...
 *
 * On the VPS that silently defeats the whole secret-handling design: runtime
 * secrets are supposed to live only in a root-owned, mode-600 `.env.production`
 * that systemd reads as root before dropping to the service user (see
 * deploy/me-platform.service). A copy inside the release is owned by the
 * deploy account and readable by the service user instead — so the build would
 * hand out exactly the credentials the deployment is arranged to withhold.
 *
