# Sonargaon University — Department of Mechanical Engineering

The public website for the Mechanical Engineering department at Sonargaon
...
notices, events, galleries, transport routes, page heroes and the footer all
come out of the database, and each has a form behind `/admin`. Editing the
site is an administrator's job, not a developer's.

**Live:** https://mechanical-engineering-olive.vercel.app
...
│   │   └── login/
│   └── api/              Auth, contact, newsletter, CSP reports,
│                         and the admin upload endpoints
├── components/
...
Vercel hosts the site, but **this project does not deploy automatically when
you push to GitHub.** Pushing updates the repository and nothing else. To put
a change live:
...
The CLI uploads the working directory, so commit or stash anything you are
not ready to publish before running it.
...
in the database* does not, so the old page can stay up for an hour. Deploy,
or make the change through the dashboard.
...
The Naval Architecture department site is the same codebase with different
content and its own database. Fixes to shared components — `PageShell`,
`Footer`, `breadcrumb.ts` — usually belong in both.
