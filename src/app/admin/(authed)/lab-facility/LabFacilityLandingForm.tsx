...
export default function LabFacilityLandingForm({ initial }: { initial: LabFacilityLanding | null }) {
  ...
  useEffect(() => {
    if (state.ok === true) toast.success('Lab Facility landing saved');
    if (state.ok === false) toast.error(state.error);
  }, [state]);

  return (
    <form action={formAction} className="space-y-4 bg-white border border-gray-200 rounded-lg p-6">
