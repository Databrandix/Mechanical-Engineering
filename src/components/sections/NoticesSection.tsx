        ...
        <div className="bg-white/5 backdrop-blur-md rounded-2xl overflow-hidden border border-white/10 shadow-2xl">
          {notices.map((notice, idx) => (
            <motion.div 
              ...
              className={`flex items-center justify-between p-6 md:p-8 hover:bg-white/10 transition-colors border-b border-white/10 last:border-0 group cursor-pointer`}
            >
              <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-12 flex-1">
                <span className="text-accent font-bold min-w-[120px]">{notice.date}</span>
                <h3 className="text-white font-medium text-lg leading-snug group-hover:text-accent transition-colors">
                  {notice.title}
                </h3>
              </div>
